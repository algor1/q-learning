...
class DQNAgent:
    def __init__(self, state_size, action_size):
        ...
        self.fit_env_loss=1
        self.stop_fit_pred_loss=0.1
        self.stop_fit_env_loss=0.1
    ...
    def _build_model_predict(self):
        ...
        model.add(Dropout(0.4))
        model.add(Dense(self.reward_size, activation='sigmoid'))
        print(model.summary())
    ...
    def _build_model_enviroment(self):
        ...
        model.add(Dense(input_dim*2, activation='relu'))
        model.add(Dropout(0.4))
        model.add(Dense(self.state_size, activation='sigmoid'))
        print(model.summary())
        model.compile(loss='binary_crossentropy',
                      optimizer='adam')
        print(model.output_shape)
    ...
    def fit_pred (self):
        batch=np.concatenate((self.memory[:,0:self.memory_ranges_state[-1]]
                                        ,self.memory[:,self.memory_ranges_state[-1]:self.memory_ranges_action[-1]]),axis=1)
        y=self.memory[:,-2]
        loss= self.model_pred.evaluate(batch,y,batch_size=self.mem_len,verbose=0)
        if loss>self.stop_fit_pred_loss:
            hist =self.model_pred.fit(batch,y,batch_size=self.mem_len,epochs=1,verbose=0)
...
if __name__ == "__main__":
    ...
    # agent.save_env("env2")  # from q32
    # agent.save_pred("pred3")  # from q52
    done = False
    ...
    for e in range(EPISODES):
        ...
        for time in range(500):
            ...
            if agent.check_answer(action).sum()<agent.check_answer((action - 1) ** 2).sum():
                action =(action - 1) ** 2
            ...
            if time >10 or done:
                ...
                if done:
                    break
                elif time>=time_prev*0.5: # последовательность не короче 50% от лучшей
